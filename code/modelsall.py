...
def _mdn_mask_fnn_model_fn(features, labels, nchannels, n_y, n_mixture, dropout, optimizer, mode, pad, lr0=1e-3, distribution='logistic', masktype='constant'):
    ...
    def _module_fn():
        ...
        # Builds the neural network
        if pad == 0:
            net = slim.conv3d(feature_layer, 32, 5, activation_fn=tf.nn.leaky_relu, padding='same')
        elif pad == 2:
            print('Valid padding')
            net = slim.conv3d(feature_layer, 32, 5, activation_fn=tf.nn.leaky_relu, padding='valid')
        #net = wide_resnet(feature_layer, 8, activation_fn=tf.nn.leaky_relu, is_training=is_training)
        #net = slim.conv3d(feature_layer, 64, 1, activation_fn=tf.nn.tanh, padding='same')
...
##
        #Predicted mask
        masknet = slim.conv3d(net, 8, 1, activation_fn=tf.nn.tanh)
        out_mask = slim.conv3d(masknet, 1, 1, activation_fn=None)
        pred_mask = tf.nn.sigmoid(out_mask)

        # Define the probabilistic layer 
        likenet = slim.conv3d(net, n_mixture*8, 1, activation_fn=tf.nn.tanh)
        net = slim.conv3d(likenet, n_mixture*3*n_y, 1, activation_fn=None)
        cube_size = tf.shape(obs_layer)[1]
        net = tf.reshape(net, [-1, cube_size, cube_size, cube_size, n_y, n_mixture*3])
#         net = tf.reshape(net, [None, None, None, None, n_y, n_mixture*3])
